...
$(function () {
  ...
  String.prototype.toTitleCase = function () {
    return this.replace(/(\w)jerk/gi, '$1 jerk')
               ...
               .replace(/(xyuv|\b[XYZ]{2,3}|[XYZ]{2,3}\b|g?lcd|yhcb|oled|\bled\b|\bmpe\b|tft|gfx|mks|pid|pwm|ssd|wyh|mmu\b|rgbw?|dgus|ftdi|pca\d+|uuid|HD44\d+|\bsd|(sd|hw|sw|ui)\b|(sc|tp)ara|\bSPI\b|cr\d+|\b[a-z]+\d+[a-z]*\b)/gi, (m, p1) => { return p1.toUpperCase(); })
               .replace(/\bMm M\b/, 'mm/min').replace(/\b0 0\b/, '0,0').replace('Gcode', 'G-code')
               .replace(/(\b(at|mm|in|of)\b)/gi, (m, p1) => { return p1.toLowerCase(); })
  }
  ...
  function commitChange(optref, fields) {
    ...
    saveWebViewState();
    ignore_update = true;
    vscode.postMessage({ type: 'change', data: optref });
  }
  ...
  function handleCheckbox(e) {
    ...
    const enabled = $cb.is(':checked');   // State based on the checkbox.
  ...
  function hideEmptySections(hasterms, $target=$formdiv) {
    ...
    // TRY: Always show the number of visible items.
    if (!hasterms) return;
    ...
    $('#filter-count').text(`${count ? count : 'No'} Setting${count != 1 ? 's' : ''} Found`);
  ...
  function addOptionLine(data, $inner) {
    ...
    if (item.depth) $linediv.addClass(`d${item.depth}`);
  ...
  function buildConfigForm() {
    ...
    const $form = $('<form>');
    for (const [sect, dict] of Object.entries(data)) {
      if (sect == '_') continue;
