...
class ConfigSchema {
  ...
  evaluateRequires(initem) {
    ...
    function priorItemNamed(name) { return self.firstItemWithName(name, initem.sid); }
    ...
    function HAS_EAXIS(eindex) {
      ...
      log(`HAS_EAXIS(${eindex})`, extruders);
      return extruders && extruders.enabled && eindex < extruders.value;
    }
    ...
    for (let the_line of text.split(/\r?\n/)) {
      ...
      if ([Parse.BLOCK_COMMENT, Parse.GET_SENSORS].includes(state)) {
        ...
        if (state == Parse.GET_SENSORS) {
          ...
          if (tline == "Temperature sensors available:") {
      ...
      // For the normal state we're looking for any non-blank line
        ...
        if (iselif || iselse || drctv == '#endif') {
          ...
          if (conditions.length) {
            function combine_conditions(cond) {
              while (true) {
                const old_cond = '' + cond;
            ...
            define_info.requires = combine_conditions(conditions.flat().join(' && '));
